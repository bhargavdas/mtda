...
class GpioPowerController(PowerController):
    ...
    def probe(self):
        ...
        for chip,pin in self.gpiopair:
            ...
            self.mtda.debug(3, "this is chip {} and pin is {} power.gpio.configure(): ".format(chip,pin))
            self.lines.append(chip.get_line(pin))

        for line in self.lines:
            try:
                if line.is_used() is False:
                    self.mtda.debug(3, "power.gpiochip{}@pin{} is free for use" .format(chip,pin))
                    line.request(consumer = 'mtda', type = gpiod.LINE_REQ_DIR_OUT)
                else:
                    raise ValueError("gpiochip{}@pin{} is in use by other service" .format(chip,pin))
            except OSError:
                self.mtda.debug(3, "line {} is not configured correctly" .format(line))
